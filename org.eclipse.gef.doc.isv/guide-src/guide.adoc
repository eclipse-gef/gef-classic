...
The `*Figures*` used for visualization of the `*EditParts*` are painted using absolute coordinates. When used together
with fractional scaling, this inevitably leads to rounding errors and visual artifacts in the editor. To compensate for
this limitation, GEF may be used in an experimental mode, where the scaling is not done by SWT, but rather by Draw2D. 

By setting the `*draw2d.disableAutoscale*` system property, auto-scaling of the `Canvas` that is hooked to the
`*EditPartViewer*` is configured without auto-scaling. Instead, the listener hooks a `*ZoomChanged*` listener to this
control and updates the internal `*monitorScale*` property whenever an event is fired. The `*RootEditPart*` listens to
this property change and updates the scale of its figure accordingly.
...
image:images/important.gif[image] Fractional scaling is currently only supported for Windows. On Linux and MacOS, this
